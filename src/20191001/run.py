...
config = {
    ...
    'image_size': (224, 224),
    'augment_prob': 0.5,
    'experiment': '20191001-5',
    'note': 'increased augmentation',
    'data_path': '../data'
}
...
def run(config, resume_training=False):
    ...
    if resume_training:
        last_ckpt = os.path.join('../data/20191001-3/weights.05.hdf5')
        model.model.load_weights(last_ckpt)
        tf.keras.backend.set_value(model.model.optimizer.lr, 0.001/5)
...
if __name__ == '__main__':
    ...
    #todo: evaluate from checkpoint
    #score = evaluate(config, model.model, df.iloc[valid_idx]) 
    #print(score)    
