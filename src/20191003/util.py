...
config = {
    ...
    'test_path_dicom': '../data/stage_1_test_images/',
    'batch_size': 64,
    'workers': 8,
    'image_size': (224, 224),
    'augment_prob': 0.5,
...
def fold_path(config, experiment, fold_no):
    return os.path.join(config['data_path'], config['experiment'], str(fold_no))

def prepare_experiment(config, makedirs=True):
    path = fold_path(config, config['experiment'], config['fold_no'])
